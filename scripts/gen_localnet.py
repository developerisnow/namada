...
parser.add_argument(
    ...
    type=str,
    help=(
        "A string representation of a dictionary of parameters to update in the"
        " parameters.toml. Must be of the same format."
    ),
)
...
if args.epoch_length:
    ...
    params["parameters"] = {"epochs_per_year": 525600}
...
system(
    f"{namadac_bin} --base-dir='{BASE_DIR}' utils init-network --chain-prefix"
    f" {CHAIN_PREFIX} --genesis-time {GENESIS_TIME} --templates-path"
    f" {TEMPLATES_PATH} --wasm-checksums-path {WASM_CHECKSUMS_PATH}"
)
...
system(
    f"NAMADA_NETWORK_CONFIGS_DIR='{temp_dir}' {namadac_bin} --base-dir='{BASE_DIR}'"
    f" utils join-network --chain-id {CHAIN_ID} --genesis-validator"
    f" {GENESIS_VALIDATOR} --pre-genesis-path {PRE_GENESIS_PATH} --dont-prefetch-wasm"
)
